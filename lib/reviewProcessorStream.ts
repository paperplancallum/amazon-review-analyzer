...
export class ReviewProcessorStream {
  private openai: OpenAI;
  private batchSize: number = 100; // Smaller batches for faster processing
  private maxBatchesBeforeConsolidation: number = 10; // Consolidate every 10 batches
  ...
  async *processReviewsStream(
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        // Intermediate consolidation to prevent memory buildup
        if ((i + 1) % this.maxBatchesBeforeConsolidation === 0 && i < batches.length - 1) {
          yield {
            currentBatch: i + 1,
            totalBatches: batches.length,
          ...
          const intermediateResults = this.consolidateResults(allResults);
          allResults.length = 0; // Clear array
          allResults.push(intermediateResults);
        }
      } catch (error) {
    ...
    yield {
      ...
      totalBatches: batches.length,
      status: 'Consolidating all insights with AI...',
      tokensUsed: totalTokensUsed,
      estimatedCost: totalCost,
      reviewsProcessed: reviewsProcessedCount,
      totalReviews: reviews.length,
